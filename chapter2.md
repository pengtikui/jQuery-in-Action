...
> 除 contents() 以外，都接受包含字符串的参数对结果进行筛选
...
`$('*').find('p cite')` 获取段落内所有 cite 元素的新包装集

#### contains(text)

返回新包装集，由包含 text 参数所传入的文本字符串的元素组成

`$('p').contains('hahaha')` 产生包装集，包含所有包含文本 hahaha 的段落

#### is()

确定包装集里是否有元素匹配传入的选择器表达式

`$('*').is('img')` 判断当前页面是否包含图像元素，返回 `ture` 或 `false`
